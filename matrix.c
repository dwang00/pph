...
void matrix_mult(struct matrix *a, struct matrix *b) {
  if (a->lastcol == b->rows){
    struct matrix *mat = new_matrix(b->rows, b->cols);
    copy_matrix(b, mat);
    for (int i = 0; i < a->rows; i++){
      for (int x = 0; x < b->lastcol; x++){
        double ans = 0;
        for (int y = 0; y < a->lastcol; y++){
          ans += a->m[i][y] * mat->m[y][x];
        }
        b->m[i][x] = ans;
      }
    }
    ...
    printf("matrix a rows must equal matrix b columns");
  }
}
